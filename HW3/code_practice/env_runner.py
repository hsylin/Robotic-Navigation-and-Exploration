	...
	def run(self, policy_net, value_net):
		...
		for step in range(self.n_step):
			...
			#TODO 3: Run a step to collect data
                        ...
                        actions, log_probs = policy_net(states_tensor, deterministic=False)
                        values = value_net(states_tensor)                        
                        self.mb_states[step, :]  = self.states
                        self.mb_dones[step, :]   = self.dones
                        self.mb_actions[step, :] = actions.detach().cpu().numpy()
                        self.mb_a_logps[step, :] = log_probs.detach().cpu().numpy()
