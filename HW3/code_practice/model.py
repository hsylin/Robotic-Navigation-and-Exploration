...
class PolicyNet(nn.Module):
	...
	def __init__(self, s_dim, a_dim, std=0.5):
		...
		self.main = nn.Sequential(
                    init(nn.Linear(s_dim, 64), nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), nn.init.calculate_gain('relu')),
                    nn.ReLU(),
                    init(nn.Linear(64, 64), nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), nn.init.calculate_gain('relu')),
                    nn.ReLU(),
                    init(nn.Linear(64, a_dim), nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), nn.init.calculate_gain('relu'))
                )

		self.dist = DiagGaussian(a_dim, a_dim, std)
...
class ValueNet(nn.Module):
	...
	def __init__(self, s_dim):
		...
		self.main = nn.Sequential(
			...
			init(nn.Linear(64, 64), nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), nn.init.calculate_gain('relu')),
			nn.ReLU(),
			init(nn.Linear(64, 1), nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), nn.init.calculate_gain('relu'))
                )
